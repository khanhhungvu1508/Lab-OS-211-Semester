    ...
    for (int i = 0; i < PAGE_TABLE_SIZE; i++)
        if (page_table[i].virtual == index) {
            ...
            *physical = ((page_table[i].physical << OFF_LEN) + offset);
            ...
            struct tlb_slot_t* least = get_least_accessed_slot();
            least->virtual = page_table[i].virtual;
            least->physical = page_table[i].physical;
            least->count = 0;
...
int main() {
    ...
    /* Init tests */
    int test[7] = {0x00003123, 0x00001524, 0x00002534, 0x17d42e52 
